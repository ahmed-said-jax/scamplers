...
fn main() {
    ...
    // The purpose of this script is to ensure that the db migrations and the rust code are synchronized and compatible with one another, as well as to apply a custom patch to the code generated by the diesel CLI. If we're in docker, it means this is a production build, and we can trust that the person compiling this app has run `compose.sh` at the root of this repo, which runs this script outside of a docker container
    if in_docker {
        return;
    }
