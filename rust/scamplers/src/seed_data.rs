...
impl SeedData {
    async fn insert(self, app_state: AppState2) -> anyhow::Result<()> {
        let mut conn = app_state.db_conn();
        match self {
            Self::DevTest {
                institutions,
                people,
                ...
                institutions.create(&mut conn).await?;
                people.create(&mut conn).await?;
                labs.create(&mut conn).await?;
                specimens.create(&mut conn).await?;
            }
