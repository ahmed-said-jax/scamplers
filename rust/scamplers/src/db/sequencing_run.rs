...
impl Create for Vec<SequencingSubmission> {
    ...
    async fn create(&self, conn: &mut diesel_async::AsyncPgConnection) -> super::Result<Self::Returns> {
        diesel::insert_into(chromium_sequencing_submissions::table).values(self).on_conflict_do_nothing().execute(conn).await?;
...
impl Create for Vec<NewSequencingRun> {
    ...
    async fn create(&self, conn: &mut diesel_async::AsyncPgConnection) -> super::Result<Self::Returns> {
        ...
        let new_run_ids: Vec<Uuid> = diesel::insert_into(sequencing_run::table).values(self).returning(id).get_results(conn).await?;

        let n_sequencing_submissions = self.iter().map(|NewSequencingRun { library_ids, .. }| library_ids.len()).sum();
        ...
        for (sequencing_run_id, NewSequencingRun {library_ids, ..} ) in new_run_ids.iter().zip(self) {
            let these_sequencing_submissions = library_ids.iter().map(|library_id| SequencingSubmission{sequencing_run_id: *sequencing_run_id, library_id: *library_id});
