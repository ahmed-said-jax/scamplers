...
impl From<diesel::result::Error> for Error {
    fn from(err: diesel::result::Error) -> Self {
        ...
        let DatabaseError(err_kind, err_info) = &err else { return Self::from_other_error(err) };
        let message = err_info.details().unwrap_or_default().to_string().replace("\"", "\'").replace("table ", "");
...
mod test_utils {
    ...
    use rstest::fixture;
    use testcontainers_modules::{postgres::Postgres as PostgresImage, testcontainers::{core::ExecCommand, runners::SyncRunner, Container, ImageExt}};
    use std::env;
    ...
    fn container() -> Container<PostgresImage> {
        ...
        PostgresImage::default().with_host_auth().with_tag(&postgres_version).start().unwrap()
    }
    ...
    fn db_conn_pool(container: &Container<PostgresImage>) -> PgConnectionPool {
        ...
        container.exec(cmd).unwrap().stdout().read_to_end(&mut Vec::new()).unwrap();

        let manager: PgConnectionManager = ConnectionManager::new(format!("postgres://{username}@{host}:{port}/{dbname}"));
