...
fn cellranger_multi_to_samples(cellranger_multi_dir: &Utf8PathBuf) -> Result<Vec<Sample>> {
    ...
    for sample_dir in per_sample_outs {
        ...
        let mut reader = csv::Reader::from_path(&metrics_summary_file).with_context(|| format!("could not read {:?}", metrics_summary_file))?;

        let first_row = reader.records().next().ok_or(Error::msg(format!("no rows found in {:?}", metrics_summary_file)))??;

        let n_cells_column = 5;

        let n_cells = first_row.get(n_cells_column).ok_or(Error::msg(format!("could not get column {n_cells_column} for {:?}", metrics_summary_file)))?;

        let n_cells: u32 = n_cells.replace(",", "").parse()?;
        
        let sample_name = Utf8PathBuf::try_from(sample_dir.path())?.file_name().unwrap_or_default().to_string();

        samples.push(Sample {name: sample_name, date_received: None, targeted_cell_recovery: None, estimated_number_of_cells: Some(n_cells)})
...
pub fn pipeline_metadata_to_data_set(pipeline_metadata_file: &PathBuf) -> Result<DataSet> {
    ...
    if pipeline_metadata.record.command == "multi" {
        ...
        for (i, metrics) in pipeline_metadata.metrics.iter().enumerate() {
            let metrics_source = metrics.get("file").ok_or(Error::msg(format!("key 'file' not found in 'metrics' object {i}")))?;
            let metrics_source = metrics_source.as_str().ok_or(Error::msg(format!("could not convert {} to str", metrics_source)))?;

            if metrics_source == "metrics_summary.csv" {
                let n_cells = metrics.get("Estimated Number of Cells").ok_or(Error::msg(format!("estimated number of cells not found in 'metrics' object {i}")))?;

                let n_cells = n_cells.as_str().ok_or(Error::msg(format!("could not convert {metrics_source} to str")))?;
                let n_cells: u32 = n_cells.replace(",", "").parse()?;
    ...
    let ds = DataSet { libraries, samples: updated_samples, lab_name: None, date_delivered: None };
